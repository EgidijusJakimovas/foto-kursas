<?php
	...
	try {
		...
		$unikalusPaymentsDB	= DB_TABLE_PAYMENTS_COLUMN_UNIKALUS;
		...
		$data = $pdo->prepare("SELECT * FROM $table WHERE $uzsakymasDB = (:response_order_id) LIMIT 1;");
		...
		if($data->rowCount() == 1){
			// Get values
			$id 			= $row["$unikalusPaymentsDB"];
			$order_id 		= $row["$uzsakymasDB"];
			$tevo1 			= $row["$tevo1DB"];
			$price 			= $row["$paidSum"];
		} 
		...
		if ($response['status'] === '1' || $response['status'] === '3') {
			...
			if ($paid == 0) {
				...
				$data2 = $pdo2->prepare("UPDATE $table SET $apmokejimasDB = ? WHERE $uzsakymasDB = ?;");
				$paid2 = 1;
